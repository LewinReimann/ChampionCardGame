...
public class FieldLayoutController : MonoBehaviour
{
    ...
    private void RearrangeCards()
    {
        ...
        float totalWidth = dynamicCardSpacing * (cards.Count - 1);

        // Position each card
        for (int i = 0; i < cards.Count; i++)
        {
            float xPosition = (dynamicCardSpacing * i) - (totalWidth / 2);
            cards[i].localPosition = new Vector3(xPosition, 0, 0);
        }
    }
}
